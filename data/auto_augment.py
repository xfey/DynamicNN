...
class AutoAugment:
    ...
    def __init__(self):
        self.policy = "v0"  # set default policy
    
    def select_policy(self, policy="v0", hparams=_HPARAMS_DEFAULT):
        if policy == 'original':
            self.policy = auto_augment_policy_original(hparams)
        elif policy == 'v0':
            self.policy = auto_augment_policy_v0(hparams)
        else:
            print("Unknown auto_augmentation policy {}".format(policy))
            raise AssertionError()
